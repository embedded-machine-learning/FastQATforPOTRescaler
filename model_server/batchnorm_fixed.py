...
class BatchNorm2dQuantFixed(nn.Module):  # TODO QUANTIZE
    ...
    def forward(self, x, in_quant=1):
        if self.training:
            ...
            with torch.no_grad():
                n = (weights_used*in_quant.view(-1))/(torch.sqrt(sig+1e-5)
                                    * self.quant.delta)
                self.n = torch.median(torch.round(torch.log2(n)))*torch.ones_like(torch.round(torch.log2(n)))  # torch.round(n)
    ...
    def get_weight_factor(self,in_delta):
        ...
        if self.first:
            # fist is set by the first true run of the class
