    ...
    def get_weight_factor(self):
        ...
        if self.training:
            with torch.no_grad():
                ...
                alpha = torch.sqrt(sig/self.sigma_for_alpha)
                # print(alpha)
...
class BatchNorm2dQuant2(nn.Module):  # TODO QUANTIZE
    def __init__(self, num_features, device=None, dtype=None):
        factory_kwargs = {'device': device, 'dtype': dtype}
        super(BatchNorm2dQuant2, self).__init__()
    ...
    def forward(self, x):
        if self.training:
            ...
            with torch.no_grad():
                ...
                # print(torch.mean(self.n))
                # + 1.0/self.quant.desired_delta
                self.t = -self.mu*n + self.bias/self.quant.delta
                self.t = torch.round(self.t).clamp(-128, 127)
