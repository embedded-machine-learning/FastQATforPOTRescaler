...
class LinQuant_(torch.autograd.Function):
    @staticmethod
    def forward(self, x, abs, bits):
        self.save_for_backward(x, abs)
        x = torch.clamp(x, -abs, abs)
        delta = 2*abs/(2.0**bits-1.0)
    ...
    def backward(self, grad_output: torch.Tensor):
        ...
        # grad_output.masked_fill(torch.gt(torch.abs(x),2**8),0)
...
class LinQuant(nn.Module):
    ...
    def forward(self, x, fact=1):
        ...
        self.desired_delta = 2*self.abs/(2.0**self.bits-1.0)
        return LinQuant_.apply(x, self.abs, self.bits)
