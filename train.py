...
def train():
    ...
    # Configure run 
    train_path = parse_data_cfg(data)["train_path"]
    test_path =  parse_data_cfg(data)["test_path"]
    ...
    # model = CamtadNet().to(device)
    # model = UltraNet().to(device)
  
    # model = model.model.TestModel().to(device)
    #model = model.model.fracBNN(batch_size).to(device)
    #model = model.model.CAMSPlit().to(device)
...
#        load_darknet_weights(model, weights)
    ...
    #     #model = torch.nn.parallel.DistributedDataParallel(model, find_unused_parameters=True)
       # model.yolo_layers = model.module.yolo_layers  # move yolo layer indices to top level
        #model.yolo_layers = model.module[-1]  # move yolo layer indices to top level
    # Dataset
    ...
    #dataset = image_loader.LoadImagesAndLabels(train_path,batch_size)
    ...
    # Testloader
    dataset_test = LoadImagesAndLabels(test_path, img_size_test, batch_size ,
                                            hyp=hyp,
                                            rect=False,
                                            cache_images=opt.cache_images,
                                            single_cls=opt.single_cls)
    #dataset_test = image_loader.LoadImagesAndLabels(test_path, batch_size * 2)
    ...
    scheduler = lr_scheduler.OneCycleLR(optimizer, 0.01, total_steps=epochs*len(dataloader))
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if epoch<5:
            model.set_Quant_IG_train_FLAG(False)
        else:
            model.set_Quant_IG_train_FLAG(False)
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni < 1:
                f = outdir + 'train_batch%g.png' % i  # filename
        ...
        if not opt.notest or final_epoch:  # Calculate mAP
            is_coco = False#any([x in data for x in ['coco.data', 'coco2014.data', 'coco2017.data']]) and model.nc == 80
            results = test.test(None,
                                None,
                                batch_size=batch_size ,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/vsc_node.data', help='*.data path')
    ...
    if not opt.evolve:  # Train normally
        try:
            ...
            print("tensorboard not supported")
            pass
